      ...
      <div class="gift-card ${cardClass}">
        ...
        <div class="gift-body">
          ...
          <div class="gift-letter-wrap" data-gift-idx="${idx}">
            <button class="gift-letter-btn" data-gift-name="${escapeHtml(g.name)}" data-gift-reason="${escapeHtml(g.reason)}" data-gift-echoes='${JSON.stringify(g._echo || [])}' data-gift-act="${escapeHtml(g.letterAct || "generic")}">✉️ 生成送礼留言</button>
...
async function handleGenerateLetter(btn) {
  ...
  const giftAct = btn.getAttribute("data-gift-act") || "generic";
  let echoes = [];
  ...
  // 降级到本地模板
  if (!result) {
    result = generateLetterFromTemplates(giftName, persona, rel, echoes, giftAct);
  }
...
// ============================================================
// 留言文案库:按礼物的"动作类型"(letterAct)写,围绕这件礼物本身能做什么
// 每个动作 4 条温暖 + 4 条俏皮,礼物名通过 ${g} 嵌入,关系语气通过 LETTER_RELATION_PREFIX 前缀点缀
// ============================================================
const LETTER_BY_ACT = {
  // 写字 / 信件 / 手账:钢笔、贺卡、笔记本、印章……
  writing: {
    warm: [
      g => `送你这支${g}。愿你用它写下的,都是让自己开心的字。`,
      g => `这支${g}给你。写日记也好,签合同也好,落笔的时候都稳一点。`,
      g => `这件${g}留给你。值得写下来的事,慢慢攒一攒。`,
      g => `送你这件${g}。话有时候不好讲,写下来反而轻一点。`,
    ],
    witty: [
      g => `这支${g}给你。字丑就用慢一点写,反正没人催。`,
      g => `送你这件${g}——不写情书也行,写日记别骂我。`,
      g => `这件${g}给你。先声明,我可不负责回信。`,
      g => `这支${g}你拿好,字好不好看是你的事,贵不贵是我的事。`,
    ],
  },
  ...
  shoot: {
    warm: [
      g => `送你这台${g}。多拍一点,后来翻起来会感谢现在这一份冲动。`,
      g => `这台${g}给你。愿它替你留住几个最不舍得忘的瞬间。`,
      g => `送你这件${g}。镜头里那些被你认真看着的人和事,都值。`,
      g => `这件${g}给你。生活有时候快得来不及看一眼,这次让它帮你按一下暂停。`,
    ],
    witty: [
      g => `送你这台${g}。第一张照片必须是我,这条要求不接受谈判。`,
      g => `这件${g}给你。拍坏了不许怪机器,拍好了记得发我。`,
      g => `这台${g}你拿好。出片不许只发朋友圈,我也要一张。`,
      g => `送你这台${g}。说好了,你掌镜,我负责好看。`,
    ],
  },
  ...
  drink: {
    warm: [
      g => `送你这件${g}。愿你每天那一杯,喝得安心,也喝得高兴。`,
      g => `这件${g}给你。忙起来记得停十分钟,煮一杯,坐一坐。`,
      g => `送你${g}。生活慢一点的时候,常常是从端起一杯开始的。`,
      g => `这件${g}留给你。值得花心思的事不多,好好喝一杯算一件。`,
    ],
    witty: [
      g => `送你这件${g}。早上不许跳过这一杯,跳过了不许说困。`,
      g => `这件${g}给你。喝完别还回来——我没那种朋友圈情谊。`,
      g => `这件${g}你收下。一个人喝是品味,两个人喝是约我。`,
      g => `送你${g}。提前声明,我对你的咖啡因摄入量不负责。`,
    ],
  },
  ...
  scent: {
    warm: [
      g => `送你这件${g}。愿你回家点上它的那一会儿,什么都不用想。`,
      g => `这件${g}给你。睡不着的夜,点一支,屋里有点味道就不那么空了。`,
      g => `送你这件${g}。希望它在你身上的味道,是你自己最喜欢的样子。`,
      g => `这件${g}留给你。烦得不行的时候,深呼吸一下也好。`,
    ],
    witty: [
      g => `送你这件${g}。说好了,点的时候不许想前任。`,
      g => `这件${g}给你。喷上之后请控制住自己,别太迷人。`,
      g => `这件${g}你收着。屋里再乱,先点上它,至少嗅觉是优雅的。`,
      g => `送你这件${g}。能解决心情问题的一半,另一半你自己想办法。`,
    ],
  },
  ...
  music: {
    warm: [
      g => `送你这件${g}。愿你戴上它的那几分钟,世界跟你没关系。`,
      g => `这件${g}给你。心情不好的时候,戴上一首歌,再说话。`,
      g => `送你这件${g}。那些没听完的歌,这次可以好好听一遍。`,
      g => `这件${g}留给你。声音放大一点,情绪也跟着松一点。`,
    ],
    witty: [
      g => `送你这件${g}。打游戏耳鸣不能怪我,音量你自己调。`,
      g => `这件${g}给你。听到好听的记得安利我,别一个人爽。`,
      g => `这件${g}你戴好。同事叫你三遍听不见——这就是我送它的意义。`,
      g => `送你${g}。歌单别太丧,人是会被歌带跑的。`,
    ],
  },

  // 数码 / 桌面:键盘、显示器、iPad、游戏机……
  tech: {
    warm: [
      g => `送你这件${g}。希望它陪你把那些想完成的事一件件做完。`,
      g => `这件${g}给你。桌面上多一件趁手的家伙,做事也松一点。`,
      g => `送你这件${g}。愿你每次打开它的时候,都是去做喜欢的事。`,
      g => `这件${g}留给你。工具好一点,人会更愿意坐下来。`,
    ],
    witty: [
      g => `送你这件${g}。它是干活的家伙,不是加班的借口,这点提前说好。`,
      g => `这件${g}给你。打游戏可以,通宵不行,寿命比键盘短的是你。`,
      g => `这件${g}你拿好。出问题别先找我——我只负责送,不负责修。`,
      g => `送你${g}。请用它做点正经的事,偶尔。`,
    ],
  },
  ...
  read: {
    warm: [
      g => `送你这件${g}。挑一本你最近最想读的,慢慢来。`,
      g => `这件${g}给你。书房里的时间,是这一年最值的时间。`,
      g => `送你${g}。希望你读到的那一句,刚好是你这阵子需要的。`,
      g => `这件${g}留给你。读得快慢都行,关键是别停。`,
    ],
    witty: [
      g => `送你这件${g}。买书如山倒,读书如抽丝——别忘了第二步。`,
      g => `这件${g}给你。读完一本来跟我聊,读不完别让我知道。`,
      g => `这件${g}你收着。当装饰也行,但你心里清楚不止这个用处。`,
      g => `送你${g}。劝你别拿来压泡面。`,
    ],
  },
  ...
  wear: {
    warm: [
      g => `送你这件${g}。第一次穿戴出门那天,愿什么都顺。`,
      g => `这件${g}给你。日常用得着的东西,是最体面的那种心意。`,
      g => `送你这件${g}。希望它能陪你走过几个还挺重要的日子。`,
      g => `这件${g}留给你。看到它的时候,也想着送它的人一秒就行。`,
      g => `送你这件${g}。冷的时候、要见人的时候、想给自己撑个气场的时候,都可以用上。`,
    ],
    witty: [
      g => `送你这件${g}。戴出去别忘了说哪儿来的——免费广告。`,
      g => `这件${g}给你。撞款的概率不大,撞人的概率取决于你。`,
      g => `这件${g}你拿好。请不要束之高阁,送礼最怕被压箱底。`,
      g => `送你${g}。穿戴上之后请勿照镜子超过十分钟。`,
    ],
  },

  // 户外 / 运动:瑜伽、滑板、瑞士军刀、望远镜……
  outdoor: {
    warm: [
      g => `送你这件${g}。下次出门或者动起来,带上它一起走。`,
      g => `这件${g}给你。世界比 14 英寸屏幕大,出去走走的时候用它。`,
      g => `送你${g}。希望它陪你去几个还没去过的地方。`,
      g => `这件${g}留给你。坚持这件事,有装备会容易一点。`,
    ],
    witty: [
      g => `送你这件${g}。请确保它的使用频率比你的会员卡高。`,
      g => `这件${g}给你。不许买完一周后压箱底——朋友监督你。`,
      g => `这件${g}你拿着。摔了别怪我,撞了别怪它。`,
      g => `送你${g}。我等着看你的"户外日记"系列发朋友圈。`,
    ],
  },
  ...
  care: {
    warm: [
      g => `送你这件${g}。再忙,也请记得每天用它五分钟。`,
      g => `这件${g}给你。身体的事不许再拖,先用上它。`,
      g => `送你这件${g}。我知道你不会主动买,所以替你买了。`,
      g => `这件${g}留给你。比起加油打气,这种小照顾更顶用。`,
    ],
    witty: [
      g => `送你这件${g}。不许嘴上说"还年轻",身体在替你说话。`,
      g => `这件${g}给你。说好了,放进柜子被发现了我可不饶你。`,
      g => `这件${g}你拿好。它能照顾你一部分,剩下的部分你自己长心。`,
      g => `送你${g}。请别再说"等忙完",我比你忙我也用。`,
    ],
  },
  ...
  comfort: {
    warm: [
      g => `送你这件${g}。累成一团的时候,抱着它发呆也行。`,
      g => `这件${g}给你。家里多一件软的,日子也会跟着软一点。`,
      g => `送你这件${g}。不为干啥,就为你回家的时候心里有点东西在等你。`,
      g => `这件${g}留给你。一个人住的夜不容易,这件陪你过。`,
    ],
    witty: [
      g => `送你这件${g}。抱着它睡可以,抱着它上班不行。`,
      g => `这件${g}给你。要是哭,记得擦干净再还回来,开玩笑的。`,
      g => `这件${g}你收着。比我有耐心,也比我软。`,
      g => `送你${g}。它的工作是治愈你,你的工作是别太作。`,
    ],
  },
  ...
  play: {
    warm: [
      g => `送你这件${g}。开心是认真要花点心思的,别忘了。`,
      g => `这件${g}给你。大人也需要玩具,别假装自己不需要。`,
      g => `送你这件${g}。希望它能让你某个无聊的傍晚,过得有点意思。`,
      g => `这件${g}留给你。事情做完了犒劳一下自己,玩半小时不亏。`,
    ],
    witty: [
      g => `送你这件${g}。拆开来玩的时候不许问我"这有啥意思"。`,
      g => `这件${g}给你。沉迷请控制,但通关请告诉我。`,
      g => `这件${g}你拿好。玩到深夜不许怪我,先警告了。`,
      g => `送你${g}。陪你 unwind 的我和它,二选一。`,
    ],
  },
  ...
  kitchen: {
    warm: [
      g => `送你这件${g}。给自己做一顿好的,是任何节日都不需要的理由。`,
      g => `这件${g}给你。希望它出现在你饭桌上的次数,比外卖多。`,
      g => `送你这件${g}。在家慢慢做一顿,人会跟着稳下来。`,
      g => `这件${g}留给你。饭桌上的小事,有时候就是大事。`,
    ],
    witty: [
      g => `送你这件${g}。第一顿做坏了,别甩锅给厨具。`,
      g => `这件${g}给你。请你做完拍照前先擦一下灶台。`,
      g => `这件${g}你拿好。等着哪天被你邀请到饭桌上,这一天必须有。`,
      g => `送你${g}。技术不好不要紧,有它撑场面就行。`,
    ],
  },
  ...
  decor: {
    warm: [
      g => `送你这件${g}。家里多一处看得见的小东西,日子就多一点重量。`,
      g => `这件${g}给你。摆在哪儿你自己挑,愿那个位置每天看一眼都喜欢。`,
      g => `送你这件${g}。希望它呆在你家里的样子,是你想要的那种安心。`,
      g => `这件${g}留给你。屋里多一件有心思的东西,空间是会跟你呼吸的。`,
    ],
    witty: [
      g => `送你这件${g}。请放在你抬头就能看见的地方,别藏柜子里。`,
      g => `这件${g}给你。要是嫌丑,送我也行,反正我喜欢。`,
      g => `这件${g}你收着。室友拍照说漂亮,记得告诉我是我送的。`,
      g => `送你${g}。今年家里的氛围,有它就有一半了。`,
    ],
  },
  ...
  memo: {
    warm: [
      g => `送你这件${g}。这一段值得被好好放进去,留给以后慢慢翻。`,
      g => `这件${g}给你。日子过得快,留下点东西是给将来的礼物。`,
      g => `送你这件${g}。希望你以后翻开它的时候,会想起几个还不错的瞬间。`,
      g => `这件${g}留给你。回忆这种东西,装进盒子里才不容易丢。`,
    ],
    witty: [
      g => `送你这件${g}。这事二十年后回头看,大概率会感谢我。`,
      g => `这件${g}给你。请认真填它,我可不接受空白档案。`,
      g => `这件${g}你拿好。以后吵架的时候翻一翻,也许就和好了。`,
      g => `送你${g}。这次别又说"以后再说",以后就是现在。`,
    ],
  },
  ...
  make: {
    warm: [
      g => `送你这件${g}。自己亲手做一件出来的感觉,买十件都换不到。`,
      g => `这件${g}给你。手忙起来的时候,脑子里那些事会自动安静。`,
      g => `送你这件${g}。慢慢做,做坏了也算自己的。`,
      g => `这件${g}留给你。做完别忘了发我看,丑的好的都看。`,
    ],
    witty: [
      g => `送你这件${g}。失败作品我也收,免费收购,放心做。`,
      g => `这件${g}给你。手作要点是"专注",朋友圈那边等做完再上线。`,
      g => `这件${g}你拿好。废了不许扔——必须当艺术品讲一遍故事再扔。`,
      g => `送你${g}。第一件做出来必须送我,这是规矩。`,
    ],
  },
  ...
  generic: {
    warm: [
      g => `送你这件${g}。不为什么特别的事,就是想到你了。`,
      g => `这件${g}给你。挑得不算多大本事,合不合你心思倒是真上了心。`,
      g => `送你${g}。生活里小心意比大场面更顶用,这件算是替我说话。`,
      g => `这件${g}留给你。希望你收下的时候,心情比这件东西本身好。`,
    ],
    witty: [
      g => `送你这件${g}。挑得不算多用力,但也没敷衍——介于中间。`,
      g => `这件${g}给你。退货是不行的,夸我可以。`,
      g => `这件${g}你拿好。礼物收下,人情记账。`,
      g => `送你${g}。请微笑着收下,即使你不喜欢。`,
    ],
  },
};

// 关系修饰:不同关系下,留言可以加一个短前缀(可选),增强语气
// 不放在 act 模板里是为了避免每个 act 都要写 5 套关系版,组合数太多
const LETTER_RELATION_PREFIX = {
  lover:     { warm: "", witty: "" },  // 留白,act 模板本身已带情感
  friend:    { warm: "", witty: "" },
  family:    { warm: "", witty: "" },
  colleague: { warm: "小心意。", witty: "同事一场——" },
  crush:     { warm: "犹豫了一下,还是想送。", witty: "假装是顺手挑的:" },
};
...
function generateLetterFromTemplates(giftName, persona, rel, echoes, giftAct) {
  const relId = rel && rel.id ? rel.id : "friend";
  const act = giftAct && LETTER_BY_ACT[giftAct] ? giftAct : "generic";
  const actPool = LETTER_BY_ACT[act];
  const prefix = LETTER_RELATION_PREFIX[relId] || { warm: "", witty: "" };

  // 默认从动作池抽一温一俏,拼上关系前缀
  let warm = (prefix.warm || "") + pick(actPool.warm)(giftName);
  let witty = (prefix.witty || "") + pick(actPool.witty)(giftName);

  // 如果有 echo 词,尝试命中场景词库(场景语境优先于动作模板)
